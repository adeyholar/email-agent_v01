// File: D:\AI\Gits\email-agent_v01\yahoo-imap-setup.js
// Yahoo IMAP Integration Setup with App Passwords
// Based on successful AOL IMAP implementation
...
async function testYahooConnection(account) {
    ...
    try {
        ...
        // Get mailbox info
        let mailboxInfo = await client.select('INBOX');
        console.log(`   📊 INBOX Status:`);
        console.log(`      📬 Total messages: ${mailboxInfo.exists.toLocaleString()}`);
        console.log(`      📩 Unread messages: ${mailboxInfo.unseen.toLocaleString()}`);

        // Test fetching recent messages
        if (mailboxInfo.exists > 0) {
            const recentCount = Math.min(5, mailboxInfo.exists);
            console.log(`   📧 Fetching ${recentCount} most recent messages...`);
            
            const messages = [];
            for await (let message of client.fetch(`${Math.max(1, mailboxInfo.exists - recentCount + 1)}:*`, {
                envelope: true,
                flags: true
            })) {
                messages.push({
            ...
            messages.slice(0, 3).forEach((msg, index) => {
                const unreadFlag = msg.flags.has('\\Seen') ? '📖' : '📩';
        ...
        return {
            ...
            unreadMessages: mailboxInfo.unseen,
            config: config
        };
...
class YahooEmailManager {
    ...
    async getAccountStats(email) {
        ...
        const mailboxInfo = await client.select('INBOX');
        await client.logout();
    ...
    async getRecentEmails(email, limit = 20) {
        ...
        if (mailboxInfo.exists > 0) {
            const startSeq = Math.max(1, mailboxInfo.exists - limit + 1);
...
async function main() {
    console.log('🚀 Yahoo IMAP Setup & Testing');
    console.log('====================================');
